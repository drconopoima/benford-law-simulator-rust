...
fn generate_sample(max_value: usize, sample_size: usize, min_value: usize, lower_boundary:u16) -> Vec<usize> {
        ...
        while current_size==pushed_values {
            for current_value in min_value..=max_value {
                ...
                if float_0_1 < target_chance {
                    if current_value>lower_boundary.into() {
...
fn upper_digits_boundary(base: u16, count_lead_digits:u32) -> u16 {
    let upper_boundary=(base.pow(count_lead_digits))-1;
...
fn main() {
    ...
    for i in lower_boundary..=upper_boundary.into() {
    ...
    Chart::new(180, 60, lower_boundary.into(), upper_boundary as f32).lineplot(&Shape::Steps(&points)).nice();
}
